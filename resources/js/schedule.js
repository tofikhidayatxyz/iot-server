...
function appendElement(data) {
	var obj  = `<div class="w-100 bg-white shadow rounded list-schedule px-2 mt-2 schedule-${data.id}" id="${data.id}">
				<div class="time">
				...
				<div class="action">
					<button class="btn p-0 ml-1 text-warning btn-edit float-right" data-id="${data.id}">
						<i class="fa fa-edit"></i>
					</button>
					<button class="btn p-0 ml-1 text-danger btn-delete float-right" data-id="${data.id}">
		...
		item.addEventListener('click',()=>{
			...
			Swal.fire({
				...
				confirmButtonColor: 'var(--info)',
					...
					loading()
					util.postData('/schedule/delete',{id:item.getAttribute('data-id')})
						.then((result)=>{

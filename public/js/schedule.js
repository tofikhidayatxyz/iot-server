...
function clear() {
  document.querySelectorAll(".swal2-confirm ,  .swal2-cancel").forEach(function (i) {
    i.removeAttribute('disabled');
  });
} // scope


document.querySelectorAll('.block-scope').forEach(function (item) {
  item.addEventListener('click', function (e) {
    document.querySelectorAll('.block-scope').forEach(function (items) {
      if (items != item) {
...
function filtrator(dom, db) {
  ...
  if (dom.length > db.length) {
    checker = _utils__WEBPACK_IMPORTED_MODULE_1__["default"].checker(dom, db);
    ...
    checker = _utils__WEBPACK_IMPORTED_MODULE_1__["default"].checker(db, dom);
...
var deleteEvent = function deleteEvent() {
  ...
  sweetalert2__WEBPACK_IMPORTED_MODULE_0___default.a.fire({
    ...
    loading();
    _utils__WEBPACK_IMPORTED_MODULE_1__["default"].postData('/schedule/delete', {
      id: item.getAttribute('data-id')
    }).then(function (result) {
...
function appendElement(data) {
  ...
  parent.innerHTML = "\n\t\t\t\t<div class=\"time\">\n\t\t\t\t\t<p class=\"text-left mb-0 time text-primary __time mr-auto\">".concat(data.time, "</p>\n\t\t\t\t</div>\n\t\t\t\t<div class=\"command px-2\">\n\t\t\t\t\t<p class=\"mb-0 text-truncate w-100 text-grey __command\">Turn <span class=\"action-name\">").concat(data.action, "</span> <span class=\"item-name\">").concat(data.target, "</span></p>\n\t\t\t\t</div>\n\t\t\t\t<div class=\"action\"></div>\n\t\t\t</div>\n\t\t\t");
...
document.querySelector('#plus').addEventListener('click', function (event) {
  ///loadingAlert()
  loading();
  _utils__WEBPACK_IMPORTED_MODULE_1__["default"].getData('/view/all', {}).then(function (result) {
    loadingStop();
...
function validator() {
  var stat = 0;
  var client = document.getElementById('target');
...
function submitSchedule() {
  var client = document.getElementById('target');
  var action = document.getElementById('action');
  var time = document.getElementById('time'); //console.log(validator())

  if (validator() == false) return false;
  loading();
  _utils__WEBPACK_IMPORTED_MODULE_1__["default"].postData('/schedule/store', {
    client: client.value,
    action: action.value,
    time: time.value
  }).then(function () {
...
function updateSchedule(id) {
  var client = document.getElementById('target').value;
  var action = document.getElementById('action').value;
  var time = document.getElementById('time').value; //console.log(validator())

  if (validator() == false) return false;
  loading();
  _utils__WEBPACK_IMPORTED_MODULE_1__["default"].postData('/schedule/update', {
    client: client.split('|')[0],
    action: action,
    time: time,
    id: id
  }).then(function (data) {
    document.querySelector(".schedule-".concat(id, " .action-name")).textContent = action;
    document.querySelector(".schedule-".concat(id, " .__time")).textContent = time;
    document.querySelector(".schedule-".concat(id, " .item-name")).textContent = client.split('|')[1];
    loadingStop();
    successAlert();
  })["catch"](function (e) {
    failAlert(JSON.stringify(e));
...
var runner = function runner() {
  ...
  var arr = [];
  document.querySelectorAll('.list-schedule').forEach(function (item) {
    var inobj = {
  ...
  if (isWait == false) {
    isWait = true;
    _utils__WEBPACK_IMPORTED_MODULE_1__["default"].getData('/schedule/all', {}).then(function (result) {
...
setInterval(function () {
  runner();
}, 1000); //* this realtime search 

document.querySelector('.form-search-object input').addEventListener('keyup', function (el) {
...
document.getElementById('update-worker').addEventListener('click', function () {
  try {
...
document.getElementById('show-drop').addEventListener('click', function () {
  document.querySelector('.dropdown-v').classList.toggle('show');
});
document.querySelector('.dropdown-v').addEventListener('click', function (event) {
  even.preventDefault();
...
function checker(val1, val2) {
  ...
  var res = check.filter(function (n) {
    return !this.has(n);
  }, new Set(data));
  check = [];
  data = [];
...
module.exports = __webpack_require__(/*! /mnt/c/Project/iot-server/resources/js/schedule.js */"./resources/js/schedule.js");
